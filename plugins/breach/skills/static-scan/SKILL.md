...
If a `custom-rules/semgrep/` directory exists in the project root (generated by `/breach:custom-rules`), include it as an additional config source:

```
semgrep --config p/security-audit --config p/owasp-top-ten --config ./custom-rules/semgrep/ --json <target>
```
...
If a `custom-rules/codeql/` directory exists in the project root (generated by `/breach:custom-rules`), include it as an additional query source:

```
codeql database analyze codeql-db --format=sarif-latest --output=results.sarif <language>-security-and-quality.qls ./custom-rules/codeql/
```
